...
After selecting the pharmaceutical to modify, press the *[.underline]##E##dit* button in the *_Pharmaceutical Browser_* window to access the *_Edit Medical_* window shown below. One can modify all the data except for the “Type” value.
...
Before closing the window with the exam result one must select a patient from the list; the fields in the *Patient's Data* panel provide some simple patient details.
...
* *[.underline]##E##xam*: choose the exam; a first window will asks for the material, then a second one the exam, finally a third one asks for the result if the exam allows only a single result, otherwise the list of multiple results are shown in the right panel, together with the chosen material.
...
Accounting is the function that is used to manage the billing process. One can enter a bill for pharmaceuticals, operations, exams, and other costs of a patient. Currently the billing process is not linked with other functions of *Open Hospital* and therefore the application does not generate the accompanying billing information based on the therapy followed by the patient: [.underline]#the billing information must be input for all items manually.#
...
Once the patient is selected one can start entering bill entries for the bill.
...
edit it. One cannot start a new bill for a patient before closing the previous one.

There are several types of Bill entries identified by the buttons in the *Buttons panel* on the right of the window:
