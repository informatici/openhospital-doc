...
TIP: It is possible to filter to find diseases more easily. To do this, enter a text fragment in the search fields that is part of the disease name. The closer the search text comes to the name of the disease being searched for, the more precise the search is.
...
* *HGT*: result of the Hemo Glucose test (in mg/dl)
...
Once *[.underline]##S##ave* button is pressed the *_Edit Admission Record_* window closes and the patient's status is changed in the *_Patient Browser_* window as
...
Next, press the *[.underline]##D##ata* button. The *_Patient Data_* window shown below appears:
...
The application is set to get the entire history of the patient, but this can be changed by querying a different date ranges and a specific set of information.
...
*[.underline]#N.B.#* There is no *Edit DICOM*, so if a mistake is made, *Delete DICOM* and re-*Load DICOM* it again.
...
To manage the therapy of a patient highlight the patient in the *_Patient Browser_* window. Next,
press the *[.underline]##T##herapy* button. The specific window for therapy management of the selected patient appears. The window has no name but it is typically referred to as the *_Therapy_* window.

In the *_Therapy_* window insertion, modification, or removal of a therapy is possible.

In addition from the *_Therapy_* window insertion or removal scheduled visits is accomplished.
...
When the therapy plan is entered (all the therapies of a patient) press the _Check Availability_ button in the *Buttons panel* on the right side of the *_Therapy_* window to check the availability of the related pharmaceuticals in the hospital. If the pharmaceutical checked is present in the quantity requested by the therapy plan green colored check mark appears.
...
Click the *[.underline]##O##K* on the button to dismiss the window.
...
When the input of the therapy plan is complete press the *[.underline]##S##ave* button in the *Buttons panel* of the *_Therapy_* window. Be aware that the therapy plan is saved when the *_Message_* window below appears:

image:image116.png[Therapy plan saved]

If the availability check was not executed (see
...
The close function exits from the *_Therapy window._* Press the *Close* button to access the Close function. The *_Select an option_* *_window_* shown below appears. The *Buttons panel* has the following choices:
...
Use these two checkboxes to set the therapies plan for this patient as important (not implemented yet) and notifiable via SMS. Briefly:
...
TIP: By pressing the *Worksheet* button a visit can be added. The instructions are the same as in
described here with the only difference being the patient is already selected and cannot be changed.
...
If two patients with a sex are merged the operation is not allowed, with the window shown below:
...
One of the Administration tasks is the setup of Open Hospital through the *[.underline]##S##ettings* functionality. It allows defining all “types” and “data” used in the current Open Hospital implementation.
...
From the *_Types_* menu the definition and management of the following types or functions are possible:
...
* *Code*: the price code and it should be unique; there are no restrictions on this field since the application uses an internal code to uniquely identify the item
...
In editing mode, the type and code cannot be changed and the only modifications allowed are to the Description and if the disease should appear in only in OPD, or Admissions, or in both.
...
In editing mode, type, code or procedure cannot be changed and the only modifications allowed are to the Description and the default value.
