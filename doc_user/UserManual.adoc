...
In general, records can be created, edited, or deleted from the DB. Certain records, however, are not deleted but allow their status to be modified, either by being "deleted," "disabled," or "marked as deleted," depending on the module.

* *Soft Deletion (Mark as Deleted)*: Soft deletion is the practice of marking a record as deleted without actually removing it from the database. The record remains stored but is typically hidden from standard operations and searches. The purpose is to retains historical or auditing data, avoids irreversible data loss and allows for potential recovery or undeleting. Generally they are hidden from standard users and workflows to avoid cluttering the user interface with irrelevant or obsolete data. Common cases is __Deleting user accounts while retaining their activity logs__. Examples include:
...
* *Disabling Records*: Disabling a record makes it inactive or unavailable for use, but it is still visible and accessible in some contexts. Unlike soft deletion, the record is often still part of workflows, albeit restricted in functionality. The purpose is to temporarily suspend a record's usage and allow for reactivation if needed. Often still visible to standard users but clearly marked as "inactive" or "disabled." to provide transparency and maintain awareness of the record's existence. Common case is __Temporarily deactivating user accounts (e.g., suspending a subscription)__. Examples include:
** <<insert-a-new-pharmaceutical,4.1.2.1 Insert a New Pharmaceutical>>: __Medicals that are marked as disabled (or inactive) will no longer appear in standard searches or operational workflows__
* *Statuses*: represent a record's inclusion in a process workflow without necessarily removing or disabling it. Examples include:
** <<delete-a-bill,6.2.3 Delete a Bill>>: __Bills that have been moved to deleted status, will continue to appear in the list, but will not be counted in the reports.__

NOTE: These features — *Soft Deletion*, *Disabling Records*, and *Statuses* — allow for precise record management across modules. They ensure flexibility while preserving data integrity and enabling workflow consistency.
