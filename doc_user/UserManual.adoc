...
* When you find text written in bold Italic (as an example *_Laboratory browsing)_* it indicates the screen with the name
...
without the use of the mouse. In the entire document the “Alt” key and the “P” key will be indicated as “Alt + P”.
...
area. Areas can be a *Selection panel*, a *Data table*, or a *Buttons panel* (see *_Laboratory browsing_* window below).
...
The example below shows all the visits for patients with Asthma admitted in the period from 2006-11-01 until
2006-11-15; all ages, all sexes, and all patient types are included.
...
Since version 1.8.4, it is possible to sort the table by any of the column headers by double clicking on the column name. Filtered type (combo box), searched keys (the aside field) and sorting are reflected in the STOCK report.
...
To delete a pharmaceutical, you have to highlight it as before. Next, press the *Dele[.underline]##t##e* button. *_The Edit Medical Record window_* appears.
...
Press the  *[.underline]##O##rder* button in the *_Pharmaceutical browsing_* window to produce the list of pharmaceuticals remaining in stock and the ones to be reordered according to their critical level.
...
Since Open Hospital version 1.9, you can quickly find a medical you are looking for. Simply type a keyword that is a part of its name into the text field at the top of the pharmaceuticals products combo box and
...
Since Open Hospital 1.8, you can perform more than one charging movement at a time. The window is composed of two areas: a *Panel* and a *Grid.*
...
Since Open Hospital 1.8, you can perform more than one discharging movement at a time. The window is composed of two areas: a *Panel* and a *Grid.*
...
The search function allows for selecting and showing laboratory exams on the *Data table* of the *_Laboratory browsing_* window.
...
* Since version *v1.11* you can also manually input the precise result (Procedure 3)
...
Since Open Hospital version 1.3.1 you can extend the _New Laboratory_ functionality by changing the _LABEXTENDED_ flag in the configuration file. Ask your Administrator how to do it.
...
Since Open Hospital version 1.4.1 you can extend the _New Laboratory_ functionality by changing the _LABMULTIPLEINSERT_ flag in the configuration file. The flag _LABEXTENDED_ has to be enabled too. Ask your Administrator how to do it.
...
Furthermore, the *_Patients Bills Management_* window shows an *Incomes Table* with following information:
...
Since Open Hospital version 1.8 you can print the Payment receipt at any time, once you have made a payment.
...
The application saves the bill and returns to the *_Patients bills management_* window; the status of the bill is “O” (Open) until you set it as PAID (*P[.underline]##a##id*).
...
Since Open Hospital version 1.7 you can print on the fly a receipt when you press the *P[.underline]##a##id* button__. To enable this feature, you must enable the RECEIPTPRINTER__ flag in the configuration file and a proper printer device must be connected to the system. Ask your Administrator how to do it or read the _Administrator’s Guide_.
...
To modify an existing bill you have to highlight it in the *_Patients Bills Management_* window. Once you have done this,
...
To delete an existing bill you have to highlight it in the *_Patients Bills Management_* window. Once you have done this,
...
Beginning with Open Hospital version 1.8 one can print the bill receipt directly from the *_Bill manager_* window. Highlight the bill
...
* *[.underline]##C##ancel* to close the window and return to the *_Patient vaccines browsing_* window without recording the patient vaccination

==== 9.2.3 Modify data of a recorded Patient vaccination ([.underline]##E##dit) 

To modify data of a Patient vaccination you have to highlight it in the *_Patient vaccines browsing_* window. Once you have done this,
...
To delete a stored Patient vaccination you have to highlight it in the *_Patient vaccines browsing_* window. Next,
...
Queries about Patients can be done using the search criteria available on the *Selection panel* area of the window (left side of the window). Results of your search are shown in the *Data table* area.
...
* *Search criteria.* You can supply the complete name of a patient or only some characters: the system will show all the patients that have the entered characters (Examples are: if you enter “solo wa” all patients that have “solo [.underline]#wa#” in the name will be selected, “Solomon Wakunga” but also “Kamwa Solonik” will be shown).
...
Once the enhanced search has been enabled the new *_Patients browser_* window looks like following:
...
To modify to data of a patient you have to highlight the patient in the *_Patient browser_* window. Once you have done this,
...
* *[.underline]##C##ancel* to close the window and return to the Menu without applying any changes

====  10.3.2 Delete a Patient (Dele[.underline]##t##e Patient)

To delete a stored patient you have to highlight it in the *_Patients browser_* window.
Next press the *Dele[.underline]##t##e Patient* button. The name of the patient is shown in the *_Delete Patient_* window below. The record can then be deleted. Deleted records will not be available anymore. _The patient is not completely removed from the system and you can ask the Administrator to restore the data at any time._
...
* *Age / BirthDate / Description*: the age can be specified in three different ways:
 +
image:image86.png[New Patient Extended_age.PNG,width=187,height=74]image:image87.png[New Patient Extended_birthdate.PNG,width=188,height=73]image:image88.png[New Patient Extended_description.PNG,width=183,height=74]
 +
Regardless of the way it is entered the age is converted to an age in years in the *_Patients browser_* window.
* *BloodType*: it can be 0+, A+, B+, AB+, 0-, A-, B-
* *Marital Status*: it can be single, married, divorced, widowed. If left empty, the default is unknown
* *Profession*: it can be other, farming, construction, medicine, food/hospitality, homemaker, mechanic, business, janitorial, mining, engineering. If left empty, the default is unknown
* *Father*: you can specify the father name and if he is still alive
* *Mother*: you can specify the mother name and if she is still alive
...
* *Load File*: could be used to load a patient picture (will be cropped and squared)
...
Since Open Hospital 1.8, the height and weight of the patient are no longer registered during patient creation. It is done through the examination module available in the *_Admission browser_* window.
...
First, to insert the admission of a patient you have to highlight the patient in the *_Patients browser_* window. Once you have done this,
...
Once you press the Save button the *_New admission_* window closes and you see the status changed in the *_Patients browser_* window
...
click the *[.underline]##E##xamination* button below. The following window will appear.
...
* *Diuresis*: Daily Urine Volume (in ml) and description
* *Bowel*: description of bowel function
* *Auscultation*: Patient auscultation. This will default to unknown if not specified. Options currently include: Unknown, Normal, Wheezes, Rhonchi, Crackles, Stridor, and Bronchial.
* *Complain*: Any additional notes concerning the examination

After typing data, Open Hospital automatically calculates the BMI and display the result on the human figure at the left.
...
To insert/modify operations data of a patient admission you have to highlight the patient in the *_Patients browser_* window. Once you have done this,
...
To discharge a patient from the hospital you have to highlight the patient in the *_Patients browser_* window. Once you have done this,
press the *[.underline]##A##dmission* button. The *_Edit admission record_* window shown above appears.
...
Once you press the Save button the *_Edit admission record_* window closes and you see the status changed in the *_Patients browser_* window
shown in the following figure:

image:image98.png[Admission Patient_discharged.PNG,width=642,height=168]

===  10.6 OPD ([.underline]##O##PD)

If the _OPDEXTENDED_ flag is set as YES in the configuration file (Ask your Administrator) the button *[.underline]##O##PD* in the Admission/Patient window will allow you to start an OPD by selecting the related patient first (see
...
This function allows modifying data about a patient and modify/delete the history of outpatients and admissions of a patient. Malnutrition can also be managed with this function.
...
The *Data table* shows you a summary of all the outpatients (OPD) visits and all the admissions related to the patient.
...
====  10.8.1 Modify data of an admission or outpatient visit ([.underline]##E##DIT)

To modify the data of an admission or outpatient visit you have to highlight it in the *Data table* of the *_Patient Data_* window. Once you have done this, press the *[.underline]##E##DIT* button. When the *_Edit admission record_* window shown below appears, the record is available for changes. All data can be changed.
...
The malnutrition control function is available for all and only the admissions that have concerns about malnutrition (malnutrition field activated in *_New admission_* window, see the next page).
...
By pressing the *[.underline]##N##ew* button in the *_Malnutrition Browser_* window, you can access the *_Malnutrition_* window below:

image:image103.png[Malnutrition Browser_edit.PNG,width=308,height=176]
...
To access the Clinical sheet function, highlight the patient in the *_Patients browser_* window.
Next, press the *[.underline]##C##linical sheet* button. The *_Patient data_* window shown below will appear.
...
To manage the therapy of a patient you have to highlight the patient in the *_Patients browser_* window. Next,
...
Therapy window. Next, press the *Remove Therapy* button and the therapy is
...
* *[.underline]#Cancel#:* to stop the function and return to the *_Therapy window_* without applying any changes
...
One of the Administration tasks is the setup of Open Hospital through the *[.underline]##G##eneral Data* functionality. It allows defining all “types” and “data” that are going to be used in the current Open Hospital implementation.
...
Once you have pressed the *[.underline]##G##eneral Data* button from the main menu you access the *_General Data_* menu. From this menu you have the following functions available:
...
Once you have pressed the *[.underline]##T##ypes* button on the *_General Data_* menu you access the *_Types_* menu:
...
|===
|image:image0_DefaultGeneralDataMenu.png[General Data Sub-Menu,width=185,height=527]|image:image0_DefaultGeneralDataTypesMenu.png[General Data Types Menu,width=235,height=707]
|===
...
Once you have pressed the *[.underline]##A##dmission Type* button from the *_Types_* menu you access the *_Disease Type Browsing_* window as shown below:
...
Once you have pressed the *[.underline]##D##ischarge Type* button from the *_Types_* menu you access the *_Discharge Type Browsing_* window as shown below:
...
Once you have pressed the *Delive[.underline]##r##y Result Type* button from the *_Types_* menu you access the *_Delivery Result Type Browsing_* window as shown below:
...
Once you have pressed the *Disease [.underline]##T##ype* button from the *_Types_* menu you access the *_Disease Type Browsing_* window as shown below:
...
Once you have pressed the *Medical[.underline]##s## Stock Mov Type* button from the *_Types_* menu you access the *_Medicals Stock Movement Types Browsing_* window as shown below:
...
Once you have pressed the *[.underline]##M##edical Type* button from the *_Types_* menu you access the *_Medical Type Browsing_* window as shown below:
...
Once you have pressed the *[.underline]##O##peration Type* button from the *_Types_* menu you access the *_Operation Type Browsing_* window as shown below:
...
Once you have pressed the *[.underline]##P##regnant Treatment Type* button from the *_Types_* menu you access the *_Pregnant Treatment Type Browsing_* window as shown below:
...
Only integer values are allowed and every range must be *contiguous* and not *overlapp* any previous or following value.
...
Once you have pressed the *[.underline]##V##accine Type* button in the *_Types_* menu you access the *_Vaccine Type Browsing_* window as shown below:
...
Once you have pressed the *Dicom Type* button in the *_Types_* menu you access the *_Dicom Type Browsing_* window as shown below:
...
Once you have pressed the *[.underline]##H##ospital* button in the *_General Data_* menu you can access the *_Hospital information_* window as shown below:
...
Once you press the *[.underline]##W##ard* button in the *_General Data_* menu you can access the *_Ward Browser_* window as shown below:
...
Once you press the *[.underline]##G##eneral Data* button in the *_General Data_* menu you access the *_Diseases Browser_* window as shown below:
...
Once you press the *[.underline]##E##xams* button in the *_General Data_* menu you access the *_Exam Browsing_* window as shown below:
...
Once you have inserted a new exam you may define the set of results it can have by first selecting the exam in the *_Exam Browsing_* window and then by pressing the *Re[.underline]##s##ults* button.
...
Once you press the *[.underline]##O##peration* button in the *_General Data_* menu you access the *_Operation Browser_* window as shown below:
...
Once you press the *[.underline]##V##accine* button in the *_General Data_* menu you access the *_Vaccine Browser_* window as shown below:
...
Once you press the *[.underline]##P##rice Lists* button in the *_General Data_* menu you access the *_Prices Browser_* window as shown below:
...
Once you press the *S[.underline]##upplier##* button in the *_General Data_* menu you access the *_Supplier Browser_* window as shown below:
...
Once you have pressed the *SMS Manager* button in the *_General Data_* menu you access the *_SMS Manager_* window as shown below:
...
image:image187.png[Login.PNG,width=305,height=148]

Once successfully logged into the system, if you have been enabled by the administrator you will find the *_[.underline]##U##sers_* button in the *_File_* submenu.

Once you press the *_[.underline]##U##sers_* button in the *_File_* submenu you can access the *_Users_* submenu as shown below:
...
Before registering a new user in the system, we should ensure there is a group with related permissions to which the user would belong.
...
Once you press the *[.underline]##U##sers* button in the *_Users_* submenu you access the *_Users Browser_* window as shown below:
...
Since Open Hospital *v1.11* a new feature called *Worksheet* allows one to manage and print the list of appointments scheduled.
