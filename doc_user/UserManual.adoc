...
* *Search by code* (work only one per time)
** *Code*: Search *ONE* OPD visit by its code, that is unique, hence only one result is expected
** *OPD No.*: Search OPD visits by their Progressive in Year, that starts every year from one, hence one or more results are expected
