...
But if the row selected is the last movement, the following dialog is presented, where a *reason* for the deletion can optionally be entered (it is kept in the audit log of deleted movements):

image:image248.png[Delete movement]

Type a reason if desired (it can be left empty) and click *[.underline]##O##K* to confirm the deletion of the last movement, otherwise click *[.underline]##C##ancel* to cancel the operation. After confirming, the system checks if the product involved in the selected movement has not been used itself in the *_Pharmaceuticals Stock [.underline]##W##ard_* (see <<pharmaceutical-stock-ward,Pharmaceuticals Stock Ward>> in this document).
