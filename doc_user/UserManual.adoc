...
The *Lot table* identifies each lot by:

* *CREATION ORDER*: the progressive position of the lot in its order of creation.
...
The pharmaceutical, the lot id and the overall quantity are read-only. The *Prep Date*, the *Due Date* and the *Cost* can be modified. Click the *[.underline]##O##K* button to save the changes: the due date cannot be before the preparation date, and the cost cannot be negative.
...
The window lists, for the selected lot, the remaining quantity in the main store and in each ward holding the lot, with the overall total in the last row.
