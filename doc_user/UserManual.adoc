...
Data that identify inventory are the following:
...
To access the other functions of the inventory in the *Buttons panel*, the following choices are available:
...
The following [.underline]##filter options## are available at the *Upper panel* of the window:
...
To record a new inventory, it must be saved with the *[.underline]##S##ave* function (see description below).

Before describing the function of the *_New Inventory_* window, the components of the window are described.
...
Inventory is composed of inventory(ies) row(s). A inventory row is a lot related to a pharmaceutical.
...
* *Status* (e.g. 'DRAFT'): the status of the inventory.
+
...
Select the desired medical or search the pharmaceutical by code or description using the search field and select the medical and click Yes.
