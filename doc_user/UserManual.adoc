...
==== 8.4.6 Administrative Issues
The *_Administrative issues_* panel of the patient form lists the issues the administration has raised on the patient and that have to be solved before the patient is served again: a missing document, an identity still to be verified, an open dispute. Each issue carries its reason and the date it was opened; while at least one issue is open the panel is bordered in red.

image:OH-patient_administrative_issues.png[Administrative issues]

Press *Open issue* and write the reason to raise a new issue; select an issue and press *Resolve issue* once it has been sorted out. Several issues can be open at the same time, and resolving one does not affect the others. The changes are saved together with the patient. Resolved issues are kept on record with the date they were closed, so that the history of the patient's issues can be traced.

Whenever a patient with open issues is selected to start a service - an admission, an OPD registration, a laboratory exam, a bill, a therapy or an examination - a warning lists the reasons. The warning is only a signal: it never prevents the operation, and any user allowed to edit a patient's record can open or resolve an issue.

The panel is meant for what the system cannot tell by itself. An outstanding balance, for instance, is already visible from the patient's bills and does not need an issue.
