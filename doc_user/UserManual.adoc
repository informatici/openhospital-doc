...
NOTE: Generally, this functionality is not allowed for regular users and should be performed only by the Administrator. Anyway, for auditing purposes, the bill will not be deleted but its status updated to "Deleted" (see <<soft-deletion-and-disabling-features,1.1.2 Soft Deletion and Disabling Features>>).
...
To mark an user as “Deleted” or restore it from “Deleted” status, check or uncheck the box after *Deleted:*. If a Group has never been used or is not linked to any previous data, the deletion will be completed with the actual record removal from the database.
(see <<soft-deletion-and-disabling-features,1.1.2 Soft Deletion and Disabling Features>>)
