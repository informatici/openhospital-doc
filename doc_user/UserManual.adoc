...
Click on the *[.underline]##I##nventory* button in the *Pharmacy* menu to access the *_Inventory Browser_* window. By default, the system shows all the inventories recorded in the period. If the "All" filter for Status is selected, the window will show "open" (draft, validated) inventories regardless the date range:
...
* *[.underline]##D##elete*: used to remove one or more products/lots from the list.
* *Clean [.underline]##T##able*: used to remove all the inventory rows.
* *[.underline]##V##alidate*: used to validate the inventory.
* *[.underline]##C##onfirm*: used to confirm the inventory.
+
...
* *[.underline]##C##lose*: used to return to the *_Inventory Browser_* window.

[#insert-inventory-row]
====== 4.4.2.1.1 Insert Inventory Row By Using The Code Or Description
 
To insert an inventory row for a medical, type into the *Add medical* field the code or the description of the pharmaceutical and press ENTER. This will open the _Choose a Medical_ window.

image:image254.png[Add a medical]

Select the desired pharmaceutical or search the pharmaceutical by code or description using the search field and select the pharmaceutical to highlight it, then click on *[.underline]##S##elect* button to add the pharmaceutical as inventory row or click on *[.underline]##C##ancel* to return to the *_Inventory Browser_* window.

====== 4.4.2.1.2 Insert Inventory Rows By Type And Usage

To insert an inventory row for a set of medicals, useful for rotational inventories, click the *Add medicals by* button. This will open the *_Lot Information_* window.
...
If the pharmaceutical is already in the Inventory with its all possible lots, the system asks if to add it anyway:

image:image255b.png[Add a medical]

By clicking on the *[.underline]##Y##es* button, the pharmaceutical will be then added to the list with a *N* flag:
...
- *Yes, all medicals*: the system will check if new movements are made on the main store for any medical and eventually show the changes in a new dialog. See below.
- *No, only the medicals in the list*: the system will check if new movement are made on the main store for the specific medicals that are in the list. E.g. see the image below. 
...
NOTE: Since the inventory will result in a number of charges/discharges in <<pharmaceutical-stock,Pharmaceutical Stock>> to align the stock levels to the real quantities (physical count), its date cannot be prior an already existing movement, so the system will always check if the current inventory is the latest operation to be recorded.

Click on the *[.underline]##O##k* button to continue. If new movements have been found, the system will show the following dialog in order to update the current inventory:
...
Otherwise, by clicking the *[.underline]##N##o* button, nothing will be changed, but the inventory will remain (or go back) to DRAFT state.

===== 4.4.2.4 Confirm Inventory ([.underline]##C##onfirm

During modification, and only once fully validated, it is possible to confirm the inventory by clicking on *[.underline]##C##onfirm* button. This will result in the following dialog:

image:image267.png[Confirm Inventory]

By clicking the *[.underline]##y##es* button, the system will finally convert all differences between real and theoretical quantities into charge/discharge movements and will inform the process has been completed with the following information message:

image:image276.png[Information]

NOTE: Every confirmation has an implicit validation, means that every time the Confirm button is clicked, the Validation is automatically clicked too before confirmation, to make sure that the inventory is fully validated before confirmation.
...
To view an inventory, highlight the inventory in the *_Inventory Browser_* window. Once the inventory is highlighted, click on the *[.underline]##V##iew* button to enter the *_View Browser_* window. 
...
NOTE: On view mode it is not possible to change anything.

===== 4.4.2.6 Print Inventory ([.underline]##P##rint)

To print the inventory, click on the *[.underline]##P##rint* button, See the resulting image below:
...
The delete button is used to cancel an inventory. Select the inventory in the *_Inventory Browser_* window, when the inventory is highlighted click on *[.underline]##D##elete* button. A confirmation window is shown:
...
NOTE: This affects the _validation_ step, when the system will check all further charges and/or discharges recorded after this date and so potentially affecting the theoretical and real quantities in the current inventory.
* *Inventory's reference*: the reference associated to the inventory.
* *Add a medical*: this field is used to add inventory row for a medical.
* *Add medicals by*: This button is used to add inventory row for a set of medicals.
...
* *[.underline]##S##ave*: this button is used to save the inventory.
* *[.underline]##L##ot*: this button is used to link a new lot to a new specific product (the inventory row that column *New Lot* as value *N*). To perform the operation, first highlighted the product and click this button.

NOTE: This button is enabled only for the inventory row where the column *New Lot* has value the value *N*, otherwise the button is disabled.
...
NOTE: The Confirm button is enabled only if the inventory is validated, otherwise the button is disabled. See 
...
To modify a ward inventory, select the inventory in the *_Manage Ward Inventory_* window. Once the inventory is highlighted, click the *[.underline]##E##dit* button to use the *return* key in the *_Edit Inventory_* window. Now the record is available for changes. This function permits editing all the data for the inventory, including inventory(ies) row(s).
...
To view a ward inventory, highlight the inventory in the *_Manage Ward Inventory_* window. Once the inventory is  highlighted, click on *[.underline]##V##iew* button to enter on the *_View Inventory_* window. 
...
Click on *[.underline]##Y##es* button, after some time the JasperViewer® will show the generated report without real quantity as follows:

image:image269.png[Report output without real quantity]

Click on *[.underline]##N##o* button, after some time the JasperViewer® will show the generated report with real quantity as follows:
