...
The Inventory features is used to manage stock levels in the *_Main Store_* as the hospital's central storage area.
...
The *_Inventory Browser_* window is composed of three areas: the *Search panel* at the top, the *Inventory table* in the center and the *Buttons panel* on the bottom. See the image below.
...
* *Charge Type*: the charge movement type used for the charge movement (when theoretic qty < real qty).
* *Discharge Type*: the discharge movement type used for discharge movement (when theoretic qty > real qty).
...
* *[.underline]##L##ot*: this button is used to link new lot to a new specific product (the inventory row that column *New Lot* as value *N*). to perform that, first highlighted the product and click on this button.
+
*NOTE:* This button is enabled just for the inventory row that column *New Lot* has value *N*, otherwise the button is disabled.

* *[.underline]##D##elete*:  used to remove one or the set of the inventory row(s).
...
+
*NOTE:* This button is enabled for the inventory after it it validated,, otherwise the button is disabled.

* *[.underline]##C##lose*: used to return to the *_Inventory Browser_* window.
+
*NOTE:* An inventory row can be inserted either for specific products or for a set of products.

====== 4.4.2.1.1 Insert Inventory Row By Using TextField
 
To insert an inventory row for a medical, enter into the field for specific product the code or the description of the pharmaceutical to insert, and press ENTER. This will open the Choose a Medical window.

image:image254.png[Choose medical]

Select the desired pharmaceutical or search the pharmaceutical by code or description using the search field and select the pharmaceutical to highlight it, then click on *[.underline]##S##elect* button to add the pharmaceutical as inventory row or click on *[.underline]##C##ancel* to back out of the *_Inventory Browser_* window.

====== 4.4.2.1.2 Insert Inventory Row Product By using SELECT Button

To insert an inventory row for a set of medicals, click on the SELECT button. This will open the Lot Information window.
...
* *Only non zero quantity*: if only all the pharmaceuticals for the selected Medical Type with non zero quantity are needed.
* *with movement only*: if all the pharmaceuticals for the selected Medical Type and which has at least one movement are needed.
...
During modification it is possible to validate the inventory by clicking on the *[.underline]##V##alidate* button.  This results in the following dialog:

image:image271.png[Error]

If you've updated the inventory and haven't saved the changes.

image:image266.png[Validate Inventory]

This Information window ask you if you check all medical on the stock or just the ones that we have on the table rows?

- If you choose yes and click on the *[.underline]##O##k* button. the system will check if new movement are made on the main store for any medical then show you this new windows if yes:

image:image272.png[Validate]

Click on the *[.underline]##O##k* button to continue and the System show you another Information windows that inform you the date of the inventory will be updated as a new movement has been found see the image below.

image:image273.png[Information]

Click on the *[.underline]##O##k* button to continue. The system ask you the last question see the picture below.

image:image274.png[Question]

If you click on the *[.underline]##Y##es* button the System will actualize the inventory by adding new inventory(ies) row(s) and update also the date time and the status of the inventory see the picture below.

image:image275.png[validate Inventory]

But, if you click on the *[.underline]##N##o* button nothing will be changed.

- If you choose No and click on the *[.underline]##O##k* button. the system will check if new movement are made on the main store for the specific medicals that are on the table rows of inventory.  

During modification it is possible to Confirm the inventory by clicking on *[.underline]##C##onfirm* button.  This results in the following dialog:

image:image267.png[Confirm Inventory]

If you click on the *[.underline]##y##es* button. the system will inform you that the process has been completed with this new information window
...
To access the functions of ward inventory, click on *[.underline]##W##ard Inventory* on the Pharmacy menu.
...
*NOTE:* The Confirm button is enabled only if the inventory is validated, otherwise the button is disabled.
