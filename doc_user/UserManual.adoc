= Open Hospital 1.11.5 - User's Guide
:icons: font
:stem:
:toc: left
:toclevels: 4
...
** *Code*: Search an OPD visit by its code, that is unique, hence only *ONE* result is expected
...
* radio box (5) to distinguish whether an individual is new (*New Attendance*)
 ...
 event is recorded as a *New Attenance* and the doctor sutures the wound and
...
The follwing [.underline]##filter options## are available at the *Upper panel* of the *_Patient Bills Management_* window:
...
* *Patient's Statement*: a report that shows a statement with all paied and unpaid bills for the selected patient
