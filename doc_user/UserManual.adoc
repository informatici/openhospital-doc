...
* the OPD number now is visible (2): it is the progressive number per year automatically set by the system; changes to the value can be made only at the first registration in a year, and then it will be increased automatically at each registration. The OPD number *MUST* be unique for each year.
...
* radio box (5) to distinguish whether an individual is new (*New Attendance*)
 ...
 time in the ambulatory; it does not matter in regard to this selection.
