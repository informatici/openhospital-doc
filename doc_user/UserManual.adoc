...
* It is compulsory to select the OPD ward (8) where the visit is taking place. Nevertheless, after first insertion in the session, the application will remember the latest selected to ease the daily data entry.
...
NOTE: for OPD and IPD wards definition, see <<ward,Ward>> settings
...
NOTE: Maternity (M) and OPD (OPD) wards are _built-in_ wards and cannot be deleted, although can be modified as preferred.
