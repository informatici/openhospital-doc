= Open Hospital 1.11.2 - User's Guide
:icons: font
:stem:
:toc: left
:toclevels: 4
...
* the OPD number now is visible (2): it is the progressive number per year automatically set by the system, changes to the value can be made only at first registration in a year, then it will be increased automatically at each registration. The OPD number *MUST* be unique for each year.
