...
The same panel also carries the administrative flag, which is not a consent. It marks a patient whose administrative position has to be solved before they are served again, for example missing paperwork, an unpaid bill, or insurance that still needs to be verified. While the flag is set the block is bordered in red, and a reason can be written below it so that the front desk knows what has to be fixed. The reason is optional, and it is cleared when the flag is removed.

image:OH-patient_consensus_administrative.png[Administrative flag]

Whenever a flagged patient is selected to start a service - an admission, an OPD registration, a laboratory exam, a bill, a therapy or an examination - a warning is shown, together with the reason when one was written. The warning is only a signal: it never prevents the operation, and any user allowed to edit a patient's record can set or clear the flag.
