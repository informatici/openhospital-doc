...
In the example below, all types of exams executed from the date 01.12.2017 to 01.10.2025 by the patient with ID equals to 558 are shown in the *Data table*.
...
** _Integer_: Valid Patient ID. If there is no patient attached to this ID, the table will be cleared.
...
After the selection, press the *[.underline]##S##earch* button; the system shows in the table results of the search applying the criteria requested. Furthermore, when pressing "ENTER" while completing the Patient ID field, the results are show considering only that field.
...
*DELETION - Medical Types cannot be deleted if still used somewhere in the application (i.e. linked to previous data) but can be marked as “Deleted” and will not be shown in the list of Medical Types in the application. If a Medical Type has never been used or is not linked to any previous data, the deletion will be completed with the actual record removal.
...
To mark an user as deleted or restore it from deleted status, check or uncheck the box after *Deleted:*.
...
To mark a user as deleted or restore it from deleted status, check or uncheck the box after *Deleted:*.
