...
NOTE: Only bills with status “O” (Open) can be modified by enabled users (See <<users-groups, User & Groups>>), otherwise, an A4 report will be shown.

If the bill, at the time of creation, was related to an admitted patient, the related ward will be shown at the upper right. 

If the patient's status has changed since then, the application will ask what to do:
...
If the answer selected is "Yes", the bill will be updated and will belong under the "OPD" category (no admission).

image:image61c.png[Edit Patient Bill OPD]

If the answer selected is "No", the bill remains unchanged and will belong to the original admission.
...
If the answer selected is "Yes", the bill will be updated and will belong to the current admission.

image:image61e.png[Edit Patient Bill Maternity Ward]

If the answer selected is "No", the bill remains unchanged and will belong under the "OPD" category (no admission)
...
Bill status can be easily identified in the last column in "Patient Bills Management" windows:
...
The Administrator, or any enabled user (See <<users-groups, User & Groups>>) can filter all incomes for all users have some payment amount:
