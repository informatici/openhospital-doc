...
- *Ward Inventory*: this feature is used to manage stock levels within individual wards of the hospital. The specifics of Ward Inventory management are explained in chapter 4.5.
...
The *_Inventory Browser_* window is composed of three areas: the *Search panel* at the top, the *Inventory list* in the center and the *Buttons panel* on the bottom. See the image above.
...
* *DATE*: indicates the date with which the inventory was entered into the system.
...
+
NOTE: this will affect the _validation_ step, when the system will check all further charges and/or discharges recorded after this date and so potentially affecting the theoretical and real quantities in the current inventory.
* *Inventory's reference*: the reference associated to the inventory.
* *Charge Type*: the charge movement type used for the charge movement (when theoretical qty < real qty).
* *Supplier*: the supplier used for the charge movement.
* *Discharge Type*: the discharge movement type used for discharge movement (when theoretical qty > real qty).
* *Destination*: the ward used for the discharge movement.
* *Status* (e.g., 'DRAFT'): the status of the inventory. This value of status is set automatically.
...
* *[.underline]##L##ot*: this button is used to link new lot to a new specific product (the inventory row that column *New Lot* as value *N*). to perform that, first select the product/lot in the list and then click on this button.
+
NOTE: This button is enabled just for the product/lot whose column *New Lot* has value *N*, otherwise the button is disabled.
...
image:image254.png[Choose a medical]
...
To insert an inventory row for a set of medicals, useful for rotational inventories, click on the SELECT button. This will open the Lot Information window.

image:image255.png[Lot Information]

Specify the Medical Type on the combo box (All = all medical types) and choose:
...
During modification it is possible to validate the inventory by clicking on the *[.underline]##V##alidate* button. This may results in the following dialog:

image:image271.png[Error]

If the inventory has not been saved yet since last changes.

image:image266.png[Validate Inventory]

This Information window asks wheter to check for any medicals on the stock or just the ones that are in the list?
...
Click on the *[.underline]##O##k* button to continue. The system may also show another information message that inform the date of the inventory will be updated as a new more recent movement has been found:
...
By clicking the *[.underline]##Y##es* button the system will actualize the inventory by adding new inventory row(s) and by updating also its date/time and its status, see the picture below:
...
During modification, and only once fully validated, it is possible to Confirm the inventory by clicking on *[.underline]##C##onfirm* button. This will results in the following dialog:
...
* *Inventory's reference*: the reference associated to the inventory.
* *Add a medical*: this field is use to Add inventory row for a medical.
* *Add medicals by*: This button is use to Add inventory row for a set of medicals.
...
This function is used to view all the data for the inventory including inventory(ies) row(s) of the inventory. There is a;sp the option to *[.underline]##P##rint* the data.

NOTE: While in view mode it is not possible to chage anything.
