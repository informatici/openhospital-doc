...
Click on the *[.underline]##I##nventory* button in the *Pharmacy* menu to access the *_Inventory Browser_* window. By default, the system shows all the inventories recorded in the period. If "All" filter for Status is selected, the window will show "open" (draft, validated) inventories regardless the date range:
...
* *Date*: the date and time when the physical check for this inventory has been done in the main store.
+
...
* *[.underline]##L##ot*: this button is used to link a new lot to a new specific product (the inventory row that column *New Lot* has value *N*). To perform that, first select the product/lot in the list and then click on this button.
+
NOTE: This button is enabled only for products/lots where the column *New Lot* has value *N*.
...
+
NOTE: The *Confirm* button is enabled once the inventory is validated, until then the button is disabled. See <<inventory-main-characteristics,Main Characteristics>> chapter.
