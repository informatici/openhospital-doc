...
Professions are no longer hard-coded: the list shown in the patient's *Profession* combo box (see <<insert-a-new-patient-extended,Insert a New Patient Extended>>) is now fully configurable from this window. A set of default professions is installed with the application and can be freely extended or modified.
