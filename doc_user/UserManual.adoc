= Open Hospital 1.12.0 - User's Guide
:icons: font
:stem:
:toc: left
:toclevels: 4
...
** *Code*: Search an OPD visit by its code; each code is unique, hence only *ONE* result is expected
** *OPD No.*: Search OPD visits by their Progressive in Year; that starts every year from 1, hence one or more results are expected
** *Pat. Id*: Search OPD visits by Patient ID, hence one or more results are expected
...
* Selecting the trash can icon (9) clears the next visit information (date, time and ward) and deletes the appointment if previously specified.
...
* In the event that the selected patient personal data need to be updated, it is possible to click on the Edit icon image:image15.png[Edit icon,width=26,height=26] near his/her name; a new window shows the patient personal information ready for modification.
...
* how many boxes of pharmaceutical remain in the stock
...
Before you close the window with the exam result, a patient must be selected from the list; the fields in the *Patient's Data* panel provide some simple patient details.
...
The relation of a bill with a patient admission is found in the last column in "Patient Bills Management" window:
...
NOTE: It is possible to have an *[.underline]##I##maging* function that allows for loading and viewing of image files for the selected patient by changing the flag DICOMMODULEENABLED in the configuration file. Ask the Administrator or refer to the _Administrator’s Guide_.
...
When you have finished entering the data, observe that the *Buttons panel* has two choices:
...
When you have finished modifying the data, observe that the *Buttons panel* has two choices:
...
When you have finished entering data, observe that the *Buttons panel* has the following choices:
...
* *Bed Days*: the number of days admitted. Computed by the system as the difference between admission date and discharge date
...
When you have finished entering all of the data, observe that the *Buttons panel* has the following choices:
...
When you have completed the changes, observe that the *Buttons panel* has the following choices:
...
The malnutrition control function is available only for the admissions who have concerns about malnutrition (malnutrition field activated in *_New Admission_* window, see the next page).
...
In the case of multi-frame image, the whole folder can be selected:
...
When you have finished entering the data, observe that the *Buttons panel* has the following choices:
...
Once you have successfully logged into the system, the *_[.underline]##U##sers & Groups_* button in the *_Settings_* submenu is available, if enabled by the Administrator.
...
Once you have selected the ward (it is possible to switch to another ward at any time), a two-column appointment schedule is shown with the appointments/services scheduled for today (on the left) and tomorrow (on the right):

image:image246.png[Worksheet with selection,width=600,height=265]

You can either select another date with the *Go to date* field, or reset the schedule back to today with the button *Today*.
