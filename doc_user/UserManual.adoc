...
press “Alt” and “P”)*.* This behavior is common thoughout the application and allows the user to operate (almost)
...
The entry (the entire line) is then highlighted in blue. This behavior is valid thoughout the application (see example below):
...
* *Type of patient.* Allows for the selection of all the visits by type of attendance: All / New / Readmittance
...
<<settings,Settings>> in this document._
 +
* *Diagnosis.* A maximum of three diseases can be diagnosed per each attendance (“new attendance” or “readmittance”
...
* Once the patient is selected and a previous OPD visit is found, special fields will be populated with the most recent visit of the patient (4), thus his/her personal data (5), the readmittance checkbox will be selected automatically (6);
...
The Lab functionality cam be extended by changing the _LABEXTENDED_ flag in the configuration file. Ask your Administrator how to do it.
...
One can also print the bill receipt directly from the *_Bill manager_* window. Highlight the bill
...
One cah optimize memory usage when the number of registered patients becomes huge, by changing the _ENHANCEDSEARCH_ flag in the configuration file. Ask your Administrator how to do it or read the _Administrator’s Guide_.
...
The height and weight of the patient are not required when registering a new patient. It is done through the examination module available in the *_Admission Browser_* window.
...
Once you press the *[.underline]##E##xams* button in the *_Settings_* menu you access the *_Exam Browsing_* window as shown below:
...
Once you press the *[.underline]##U##sers* button in the *_Users & Groups_* submenu you access the *_Users Browser_* window as shown below:
