...
- Status workflow: DRAFT (save) -> VALIDATED (Validate) -> DONE (Confirm)
...
* *Add medical*: this field is used to add a single product by typing its code or name.
...
- *No, only the medicals in the list*: the system will check if new movement are made on the main store for the specific medicals that are in the list.  
...
NOTE: the inventory date, as it will result in a number of charges/discharges in <<pharmaceutical-stock,Pharmaceutical Stock>> to align the stock levels to the real quantities (physical count) cannot be prior an already existing movement, so the system will always check if the current inventory is the latest operation to be recorded.
...
By clicking the *[.underline]##Y##es* button the system will actualize the inventory by adding new inventory row(s) and update also the date time and the status of the inventory, see the picture below:

image:image275.png[validate Inventory]

Otherwise, by clicking the *[.underline]##N##o* button, nothing will be changed, but the 

During modification it is possible to Confirm the inventory by clicking on *[.underline]##C##onfirm* button. This will results in the following dialog:
...
The delete button is used to remove an inventory. Highlight the inventory in the *_Inventory Browser_* window, when the inventory is highlighted click on *[.underline]##D##elete* button. A confirmation window is shown:
...
The Inventory feature is used to manage the stock levels in a *_Ward_*.
...
All the functions available under *[.underline]##W##ard Inventory* are accessible from the *_Manage Ward Inventory_* window. By default, the system shows all the inventories recorded in the period.
...
The *New Inventory* function is used to create a new inventory on the main store. Click on *[.underline]##N##ew* button in the *_Manage Ward Inventory_* window to access the *_New Inventory_* window.
...
* *Ward*: the ward that is associated with the new inventory.
* *Date*: the date of the inventory.
* *Reference*: the reference associated to the inventory.
* *Add medical*: this field is use to Add inventory row for a medical.
...
To modify a ward inventory, highlight the inventory in the *_Manage Ward Inventory_* window. Once the inventory is highlighted, click the *[.underline]##E##dit* button to use the *return* key in the *_Edit Inventory_* window. Now the record is available for changes. This function permits editing all the data for the inventory, including inventory(ies) row(s).
