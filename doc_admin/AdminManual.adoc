...
Depending on the operating system you have, there are several software tools to install and manage a MySQL server. In most cases just a standard installation package of the software is needed, paying attention to configuration options during the installation process.
...
NOTE: Starting with Open Hospital 1.11, MariaDB is the default choice.
...
If deployed in a client-server, networked configuration, MySQL server must be configured to listen on the network interface connected to the local LAN; this can be achieved by changing MySQL's config file, “my.cnf”, and commenting out the "bind-address" parameter (using the *#* character)
...
In this way the database server will be listening on any IP address (0.0.0.0) configured on the server and can serve the connecting clients.
...
Create the database user with password, the main OH database and grant the necessary permissions with the following SQL commands (you can adjust database name, user and password according to your specific setup):
...
In order to check if the database has been correctly created, you can use the command:
...
+--------------------+
----
You are now ready to launch the main SQL script “create_all_en.sql” with the command:
...
Open Hospital comes with several configuration files called “properties files”. In order to let Open Hospital find its database, we have to modify the _database.properties_ and _log4j.properties_ files, located in the “resource” folder rsc/.
...
NOTE: If you want to use a Open Hospital portable for Linux from a USB key you need to make sure that the file permissions in the
...
In Open Hospital portable for Linux you will find the following folder tree structure in the downloaded archive, where *_<version>_* is replaced with the current Open Hospital version and *_<arch>_* with the OS architecture (x32 or x64):
...
In Open Hospital portable for Windows you will find the following folder tree structure in the downloaded archive, where *_<version>_* is replaced with the current Open Hospital version:
