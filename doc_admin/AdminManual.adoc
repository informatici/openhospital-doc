...
|===
|Key |Default Value |Valid Values
|PATIENTPHOTO |DB |DB, filesystem path
|===

Open Hospital can save profile picture on DB or on filesystem. 
When it is not specified DB but a non empty string, Open Hospital will search for profile picture in the path specified in PATIENTPHOTO parameter.
