...
The software acts as a client application. The server part utilizes a DataBase Management System (DBMS) to archive, share and present the information. The free and open-source MariaDBfootnote:[See https://www.mariadb.org] MariaDB is the database server of choice.
...
For the MariaDB/MySQL installation and database creation refer to the chapter <<mysql-server,MySQL Server and Open Hospital DB>> and the online documentation.
...
Since the JRE and MariaDB/MySQL are not platform-independent and come in different versions depending on the platform, there exists:

* *Open Hospital portable for Linux* platforms (embedding Java JRE and MariaDB for Linux, 32 or 64bit)
* *Open Hospital portable for Windows* platforms (embedding Java JRE and MariaDB for Windows)
