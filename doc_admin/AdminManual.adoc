...
|===
|Key |Default Value |Valid Values
|AUTOCOMPILEJRXML |no |yes, no
|===

Open Hospital ships precompiled _.jasper_ reports and loads them at report time. When a report template (_.jrxml_) is edited on disk but its _.jasper_ is not regenerated, report generation fails with a stale or missing compiled report.

With *AUTOCOMPILEJRXML = yes* the application scans the report folders (see <<reports,Reports>>) at startup and (re)compiles every _.jrxml_ whose _.jasper_ is missing or older than the template, so that keeping the _.jrxml_ up to date is enough. A template that fails to compile is logged and skipped, without stopping the others. By default, AUTOCOMPILEJRXML is set to no.

WARNING: JRXML templates can contain executable expressions. Only enable this option when the report files are trusted, that is, not writable by untrusted users.

NOTE: The compiler expects the templates in the JasperReports 7 format; legacy JasperReports 6 templates must be migrated first (for example with Jaspersoft Studio).
