...
The reader will not find in this manual the information one needs to use the application software; that information is found in our _User’s Guide_, supplied with the application software.

== 2 Introduction

The aim of this program is to manage, in the simplest manner, the hospital administrative operations like registering patients and laboratory analysis, and to produce statistics for the health ministry. In the following chapters the users will find all the information they need in order to use the software properly.

== 3 Installation

=== 3.1 OpenHospital

OpenHospital is a stand-alone Java software program that does not require installation. The .zip file can be copied and moved everywhere in your system, whether it is Windows or Linux.

It acts like a client application. The server part is carried out by the DBMS (Database Management System) that is, from its first version, the free and open-source MySQL database.

The real part of the installation concerns the JRE (Java Runtime Environment) installation, the MySQL Server and the creation of the DB, which is done through the execution of a script in the folder mysql/ within the .zip file folders.
...
*_Warning_*: _Since OpenHospital v1.11 Java6 (Java 1.6) is no longer supported!!! Minimum requirement is Java8 or greater (1.8 or greater). If you see the application small-scaled, probably you are using an high-dpi display, in such case there are some solutions https://stackoverflow.com/a/39372897/5309541[click here]) to apply only on the machines using this kind of monitors._


* [line-through]#Install the Java Runtime Environment 1.6 or higher (1.6 is recommended for fully compliance, and it’s the only one that supports high dpi monitors)#
...
*_NB:_* _OpenHospital v1.10 or lesser, require a JRE 1.6 or higher to work properly. Anyway, at the time of writing this document, *only JRE 1.6 32bit* has been tested and it is fully compliant with all OpenHospital features, especially the ones hardware related (such as Webcam and SMS)._

==== 3.1.2 MySQL Server and OpenHospital DB

Depending on the operating system you have, there are several pieces of software to download to install and manage a MySQL server. In most cases one downloads a standard installation file of the software, with some aspects to pay attention during the installation process.

For instance, on a Microsoft Windows® system you will have MySQL Workbench® as a client, while in Linux based systems you may have MySQL Administrator and MySQL Query Browser to perform the same tasks or even MySQL Workbench with newer distros.
...
* *on a Windows machine*: during the installation it will ask for several configuration options; the best configuration will be (it could differ a bit):
...
that means that we successfully connected to the MySQL Server instance just installed with the root user and it is waiting for our commands.
...
_Improved performance in a production environment can be achieved by changing following parameters in the MySQL config file “my.cnf”:_
...
_For this reason, if the DB is shared between different operating systems it is recommended to change the following parameter in the MySQL config file “my.cnf” of each system, as in http://dev.mysql.com/doc/refman/5.0/en/identifier-case-sensitivity.html_
...
_*[.underline]#NB#*: These settings are required for every OpenHospital copy, that is to say, on every client._
...
OpenHospital 1.7 and greater versions, come with the generalData.properties file set as follow by default:
...
OpenHospital has a predefined user called “admin” that has all rights to access all functionality in the application. When SINGLEUSER is set to YES (default) the program starts with this user and all the functionalities are available.
...
Once logged in as the “admin” user you can define users and group (see link:#_Users_and_Groups[Users and Groups (Users)]), or just continue with a simple form of data protection.
...
*[.underline]#Disclaimer#*: [.underline]#Data security must never be left solely to application but it must include proper network architecture and a rigid configuration of the clients.#
...
Since version *v1.11.0*, the _AUTOMATIC LOTS MANAGEMENT_ has been split into two different settings for charging and discharging the main pharmacy and to manage lots also in the wards. In this way we offer more control to the user that may want to provide all the lots details in _charging operations_ but ask the application to automatically _discharge_ the nearest expiring ones (FEFO). Similarly, in wards (See ch. link:https://github.com/informatici/openhospital-doc/blob/develop/doc_user/UserManual.adoc#6-3-pharmaceuticals-stock-ward-pharmaceuticals-stock-ward[6.3 Pharmaceuticals Stock Ward] in User's Manual) the user may decide to manage lots manually or to ask the application to do it automatically.

With *AUTOMATICLOT_IN = YES* we ask the application to generate a lot number automatically and only ask for an expiration date (compulsory). By default, AUTOMATICLOT_IN is set to NO. It is possible to change the value at any time.

With *AUTOMATICLOT_OUT = YES* we ask the application to discharge automatically the nearest expiring lot when required. By default, AUTOMATICLOT_OUT is set to NO. It is possible to change the value at any time.

With *AUTOMATICLOTWARD_TOWARD = YES*, we ask the application to discharge from Ward (to other Wards) the nearest expiring lot automatically when required. By default, AUTOMATICLOTWARD_TOWARD is set to NO. It is possible to change the value at any time. For discharges to Patients (drugs giving) the lot is *never* asked.

_*[.underline]#NB#*: An application restart is required in order to see the modification applied for all above settings._
...
OpenHospital, since version 1.10, allows to enable or disable thumbnails in the Dicom Viewer.
...
OpenHospital 1.7 and greater comes with the database.properties file set as follow by default:
...
NB: Same settings have to be applied in log4.properties (since OpenHospital 1.7); see link:#5-6-log4j-properties[log4j.properties] in this manual._

=== 5.3 dicom.properties

OpenHospital 1.8 and greater versions, come with the dicom.properties file set as follow by default:
...
* *dicom.max.size:* is the max allowed size for a dicom/jpg image (the mysql server setting max_allowed_packet should be set with the same or a greater value). If missing default value is "4M"

=== 5.4 examination.properties

OpenHospital 1.8 and greater versions, comes with the examination.properties file used in the examination module. The file is set as follow by default:
...
OpenHospital 1.7 and greater versions, come with the log4j.properties file set as follow by default:
...
OpenHospital 1.8 and greater versions, come with the sms.properties file set as follow by default:
...
OpenHospital 1.8 and greater versions, come with the GSM.properties file set as follow by default:
...
OpenHospital 1.8 and greater versions, come with the Skebby.properties file set as follow by default:
...
OpenHospital 1.7 and greater versions, come with the txtPrinter.properties file set as follow by default:
...
Bundles are the language (or translation) files provided with OpenHospital. As told in link:#5-1-1-language[LANGUAGE] chapter, OpenHospital 1.7 and greater versions, come with different available languages in folder bundles/ within the package file:

*language_XY.properties*

where XY is an international country code.

They are property files (basically text file), made by hundreds of *key = value* couples.
...
For instance, if you want to create an Arab translation, you may create a new bundle from the English one, by copying _language_en.properties_ into a *new file* _language_ar.properties_.
...
*_Warning_*: _Since version 1.9.1, the code is available on GitHub at these links:_

* https://github.com/informatici/openhospital-core/tree/v1.11.0[openhospital-core]
* https://github.com/informatici/openhospital-gui/tree/v1.11.0[openhospital-gui]
* https://github.com/informatici/openhospital-doc/tree/v1.11.0[openhospital-doc]

_Please use EGit (Eclipse Plugin for Git) in order to clone the code into your Eclipse)_

Assuming Eclipse EE, MySQL 5.0.x and the Java Virtual Machine 1.8 or higher (Java8 or greater) have already been installed on the computer, this chapter will focus on cloning the version 1.11 on Eclipse J2EE (or Eclipse EE) environment.
