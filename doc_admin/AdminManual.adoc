...
* *data/images* – contains the image files
...
NOTE: The imaging viewer was previously referred to as DICOM viewer.
...
Default imaging configuration is available in the _dicom.properties.dist_ file:
...
this property specifies the path where the image files are stored. It is possible to specify a local folder (_data/image_storage_ is the default path under the OH installation folder) or to set a shared network folder, useful for LAN - client/server environment. Suggested configuration is to map the network/NFS folder under the _data/image_storage_ path with a symbolic link.
...
* *DICOM_SIZE*: maximum allowed size for an image (the MariaDB/MySQL server setting max_allowed_packet should be set with the same or a larger value).
...
dicom.storage.filesystem=Z:/OH/Shared/patient_images
dicom.max.size=4M
----
