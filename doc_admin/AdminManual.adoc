...
[Open Hospital][openhospital] (OH) is a free and open-sourcefootnote:[Open-source = software for which the original *source code* is made available to anyone free of charge.] Electronic Health Record (EHR) software application.
Open Hospital is deployed as a desktop application that can be used in a standalone, single user mode (PORTABLE mode)
or in a client-server network configuration (CLIENT mode), where multiple clients and users can connect to the same database server.
