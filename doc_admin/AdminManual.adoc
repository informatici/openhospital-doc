...
=== 2.2 OH - SERVER mode (portable)

Open Hospital can be lauched in SERVER (portable) mode. SERVER mode allows to launch a local instance of the database server and listen for connections from other clients.
It is useful to test/simulate a client/server configuration or networked environment; it is not suggested for production setup.
To setup a complete client / server configuration, refer to chapter <<database-server,database server>>
...
|LANGUAGE |en |ar, de, en, es, fr, it, fr, pt, sw, zh_CN
|===
