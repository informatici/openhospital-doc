...
Reports in Open Hospital are produced with JasperReports® technology. JasperReports gives the possibility to design a report in an visual / WYSIWYG (What You See Is What You Get) editor, connect it to a datasource (DB), test it, modify it, and then compile it to use it directly from the OpenHospital interface.

Open Hospital reports are placed into three different folders (see <<folders,Folders>> in this document):

* rpt_base/ -> contains all the hardcoded reports used by the application
* rpt_stat/ -> contains the //OHxxx// basic reports available in the Statistics menu
* rpt_extra/ -> contains additional reports defined by the user. Users can create new custom reports, copy them to the folder and the application will show the custom reports in the Statistics menu.
    
Each report consists of two files:

* a .*jrxml* file: JasperReport XML file, that can be modified with a specific editor
...
With Jaspersoft® Studio it is possible to open existing Open Hospital reports, edit and re-compile them in jasper format; the jasper file can then be placed in the same folder and used by the Open Hospital application.
