...
NOTE: Before running the project, the database must be created as explained in
...
If the database upgrade procedure is successful, it is possibile to update the OH software by replacing the OH working copy on each client with the new one. 
(see <<database-properties,database.properties>> and <<log4j-properties,log4j.properties>>).
