...
[Open Hospital][openhospital] (OH) is a free and open-sourcefootnote:[Open-source = software for which the original *source code* is made available to anyone free of charge.] Electronic Health Record (EHR) application.
Open Hospital is deployed as desktop application that can be used in a standalone (portable), single user mode
or in a client-server network configuration, where multiple clients and users can connect to the same database server.
Open Hospital is the first of a set of software products that ISFfootnote:[Informatici Senza Frontiere = Computer scientists without frontiers] has developed to support the information management and the activities of hospitals and health centers.
...
Open Hospital is developed in Java and it is based on open-source tools and libraries; it is designed to work without an internet connection and requires low computer resources.
...
This document describes how to install and configure the application. This includes enabling and disabling functionality as well as managing users and groups.

The reader of this manual will not find the information needed to use the application software; that information is found in the _User’s Guide_, supplied with the application software.

<<<

== 2 Introduction

The aim of this program is to manage, in the simplest manner, the hospital administrative operations like registering patients and laboratory analysis, and to produce detailed statistics and reports. In the following chapters all the information needed to install and configure the software are presented.
...
Open Hospital is a stand-alone Javafootnote:[Java is a registered trademark of Oracle and/or its affiliates.] software program that does not require installation and is often known as a portable installation. The .zip file can be copied and moved everywhere on the target system, whether the operating system is Windows or Linux.

The software acts as a client application. The server part utilizes a DataBase Management System (DBMS) to archive, share and present the information. The free and open-source MySQLfootnode:[See https://www.mariadb.org] MariaDB is the database server of choice.

The procedure includes the installation the JRE (Java Runtime Environment), the MySQL Server and the creation of the database through the execution of a script in the folder sql/ within the .zip file folders.

The typical client/server, network (LAN) configuration is shown in the following figure:
...
For the MySQL/MariaDB installation and database creation refer to the chapter <<mysql-server,MySQL Server and Open Hospital DB>> and the online documentation.
...
TIP: Some tasks can be automated using the new oh.sh / oh.ps1 scripts.

==== 3.1.1 Prerequisites - Software compatibility matrix

Before starting, check the following software compatibility matrix, in order to verify the external software versions needed to run Open Hospital and the current available extended features for every version, architecture and operating system.
...
Depending on the operating system, there are different ways to install a Java Virtual Machine and the Java Runtime Environment (JRE).
The oh.sh / oh.ps1 set of scripts can download and install the latest JRE available by downloading it from internet; it is also possibile to install
manually, following the specific instructions for your operating system.

NOTE: See Software Compatibility Matrix in order to identify the correct Java version.

TIP: If when running Open Hospital the application appears small-scaled or blurry then the issue is probably using a high-dpi display. In such cases there are some solutions (https://stackoverflow.com/a/39372897/5309541[click here]) to apply only on the machines using this type of monitor.

[#mysql-server]
==== 3.1.3 Database server configuration

Depending on the operating system, there are several software tools to install and manage a MySQL server. In most cases just a standard installation package of the software is needed, paying attention to configuration options during the installation process.

For instance, on a Microsoft Windows® system, MySQL Workbench® can be used as a client to connect to the database server and perform the needed tasks, while on Linux based systems the MySQL Administrator and MySQL Query Browser tools are used to perform the same tasks. Some newer distributions (distros) include MySQL Workbench.

The suggested approach for installation is to use the command-line MySQL client which uses essentially the same commands and syntax for every platform.

NOTE: See Software Compatibility Matrix in order to identify the correct MariaDB / MySQL version.

NOTE: MariaDB is the preferred alternative for the DBMS.

===== 3.1.3.1 MySQL - Server installation

Please referer to the latest MySQL Server online documentation for downloadfootnote:[https://mariadb.org/download/] and installationfootnote:[https://mariadb.org/documentation/] information while using this document.
...
===== 3.1.3.2 MySQL - Configuration

Please use the following configuration values for MySQL server in my.cnf config file. Values can be adjusted and adapted to the hardware resources available.
...
===== 3.1.3.3 MySQL - Networking

If deployed in a client-server, networked configuration, the MySQL server must be configured to listen on the network interface connected to the local LAN; this can be achieved by changing MySQL's config file, “my.cnf”, and commenting out the "bind-address" parameter (using the *#* character)
...
[source,shell]
----
C:\WINDOWS> cd D:\OpenHospital-v1.11.0\sql

D:\OpenHospital-v1.11.0\sql
...
[source,shell]
----
D:\OpenHospital-v1.11.0\sql> mysql -u root -p
Enter password: *****
----
...
In order to check if the database has been correctly created, enter the following command:
...
NOTE: The data can be installed with different languages by using the related "create_all_xx.sql" file, or with demo data using the "create_all_demo.sql" file (English only).
...
The script creates the OH database structure and populates it with default data.
...
Open Hospital comes with several configuration files called “properties files”. In order to let Open Hospital find its database, modify the _database.properties_ and _log4j.properties_ files, located in the “resource” folder rsc/.

Specify:

* the *IP address* of the machine where the MySQL Server was installed and containing the Open Hospital database. Replace “localhost” with the server IP address (e.g., 192.168.1.100)
* the “isf” (or the user specified in the installation phase) user *password*
* the MySQL Server *Port Number*, if changed from “3306”

NOTE: These settings are required for every Open Hospital copy / client.
...
* *on a Windows machine*: oh.bat (Select Client mode)

If everything is working, the Open Hospital main splash screen is presented:
...
=== 3.2 Open Hospital - Portable

Open Hospital is also distributed in a *portable* mode.

Since Open Hospital is a Java software program, it is portable by definition. The term “portable” here refers to the possibility of having a self-contained copy of the software that includes Java, MySQL Server, and the database. This package can be copied and run everywhere, carrying together all the saved data.

Since the JRE and MySQL are not platform-independent and come in different versions depending on the platform, there exists:

* *Open Hospital portable for Linux* platforms (embedding Java JRE and MySQL for Linux, 32 or 64bit)
* *Open Hospital portable for Windows* platforms (embedding Java JRE and MySQL for Windows)

TIP: If when running Open Hospital the application appears small-scaled or blurry then the issue is probably using a high-dpi display. In such cases there are some solutions (https://stackoverflow.com/a/39372897/5309541[click here]) to apply only on the machines using this type of monitor.

Once the required version is downloaded, the application can be started by launching the related script inside the package:
...
NOTE: To use Open Hospital portable for Linux from a USB key it is required that the file permissions in the
...
NOTE: It is important to understand that the portable version is meant to be a trial, that is, to try or test the software
or to be used in a very small facility (like a dispensary) with only one computer. If the facility is large and needs a networked
...
Backup of the Open Hospital database can be performed in several ways. The easiest way is to use the provided scripts (oh.sh/oh.ps1) by choosing the option "-s":

Restore of the database is also possible; See script help form more informations.

[#backup-data]
==== 3.3.1 Backup of database (alternative with MySQL Workbench®)

There are several ways to back up and restore *data* with MySQL. The recommend method is to use a common MySQL client such as MySQL Workbench®:
...
Backup of the software *code and settings* is needed only if changes have been made to the “official version” to
...
* *sql* – contains the scripts and configurations for MySQL database
...
In the Open Hospital zip file the following folder tree structure is found in the downloaded archive, where *_<version>_* is replaced with the current Open Hospital version:
...
=== 4.2 Folder tree in Open Hospital portable for Linux (32 and 64bit)

In Open Hospital portable for Linux the following folder tree structure is found in the downloaded archive, where *_<version>_* is replaced with the current Open Hospital version and *_<arch>_* with the OS architecture (x32 or x64):

* *OpenHospital-_<version>_-linux_i686-portable*/ or *OpenHospital-_<version>_-linux_x86_64-portable*/
** data/ - folder for database, DICOM files and logs
** etc/ - working folder
...
** LICENSE - GNU GENERAL PUBLIC LICENSE V3
** oh.ico - icon image for launcher
** *oh.sh Launcher for OH-Linux*
** POH-linux-changelog.md (changelog portable-linux)
** POH-README.md (README portable-linux)

=== 4.3 Folder tree in Open Hospital portable for Windows (32bit)

In Open Hospital portable for Windows the following folder tree structure is found in the downloaded archive, where *_<version>_* is replaced with the current Open Hospital version:
...
The default password for the “admin” user is “admin”. After typing this password in the white field and pressing *[.underline]##S##ubmit*, the main *_Menu_* with all functionalities are enabled.
...
If it believed that the PORT is correct but still Open Hospital is not communicating with the device (try switching to DEBUG log level – or lower - during this setup –
...
XMPP Server separately on the OH Server, typically on the same machine as the MySQL Server.
...
Every new log into Open Hospital, will create an OpenFire user with the same username and password (even if passwords are saved
...
. Consideration should be give to using the OpenFire plugin to automatically add new users to a group (e.g., registration plugin http://www.igniterealtime.org/projects/openfire/plugins/registration.jar)
...
Assuming Eclipse EE, MySQL/MariaDB and the Java Virtual Machine 1.8 or higher (Java8 or greater)
...
as default. This can be verified by the “SINGLEUSER=yes” string on the third row of the <<settings-properties,settings.properties>> file.
...
[source,shell]
----
C:\WINDOWS\system32> cd D:\OpenHospital_1_11_0\sql

D:\OpenHospital_1_11_0\sql>
----
...
[source,shell]
----
D:\OpenHospital_1_11_0\sql> mysql –u root -p
...
[start=6]
. Then launch all the scripts in order that differ from the previous version; for example:
...
[start=7]
. Take notes if any error message occur and report them to: +
...
. Start the program and check from log files (OpenHospital_1_11_0/logs) to ensure the new version running and has no error messages.

=== 8.2 Update Open Hospital - portable

NOTE: Open Hospital portable is not meant to be used in a production environment so to update a portable version and keep the previous data, it is strongly suggested switching to a full installation as follows:
...
* A brief history of the installation and the use of Open Hospital
* The current Open Hospital version and the new version to move to
