= Open Hospital 1.15.1 - Administrator’s Guide
<<<
:icons: font
:stem:
:toc: left
...
:xlmnsBySa: https://purl.org/dc/terms/
...
pass:[<br>][.small]#Administrator’s Guide, &#169; 2026 by https://www.informaticisenzafrontiere.org/[Informatici Senza Frontiere Onlus]#
...
Open Hospital is the first of a set of software applications that ISFfootnote:[Informatici Senza Frontiere - https://www.informaticisenzafrontiere.org] has developed to support the information management and the activities of hospitals and health centers in the simplest manner possible, by providing tools for the administrative operations (like registering patients, managing laboratory analysis and pharmaceutical stocks) and to produce detailed statistics and reports.
...
After uncompressing the package, browse to the extracted directory (example given for version 1.15.0):

* on Linux:
----
cd /home/OH/OpenHospital-v1.15.0
----
* on Windows:
----
cd C:\Users\OH\OpenHospital-v1.15.0
----
...
installation is presented, including procedures on how to enable and disable features as well as manage users and groups in a multi-user environment.
More information can be found on the Open Hospital website https://www.open-hospital.org/documentation.
...
* <<client,2.1 OH - CLIENT mode>>: multi-computer setup, with a central database (requires some IT knowledge)
...
** <<setup,Test>> and <<run,run>> the application using the 'localhost' setting
* Clients
...
[source,shell]
----
C:\WINDOWS> cd C:\Users\OH\OpenHospital-v1.15.0\sql

C:\Users\OH\OpenHospital-v1.15.0\sql
...
[source,shell]
----
C:\Users\OH\OpenHospital-v1.15.0\sql> mysql -u root -p [-h hostname/ip address]
Enter password: *****
----
...
*Alternative method*: you can first generate the config files with the *-g* and after directly modify the _database.properties_ and _log4j2-spring.properties_ configuration files, located in the _oh/rsc/_ resource folder, without generating them every time.
...
in _log4j2-spring.properties_ config file
```
...
Open Hospital has a folder tree hierarchy which is shown here, where *_<version>_* is replaced with the current Open Hospital version, *_<operating_system>_* with windows or linux and *_<arch>_* with the OS architecture (32 or 64bit):
...
* *oh/rpt_base/<lang>* – Language-specific translation files for reports in oh/rpt_base
...
* *oh/rpt_stat/<lang>* – Language-specific translation files for reports in oh/rpt_stat
...
* *oh/rpt_extra/<lang>* – Language-specific translation files for reports in oh/rpt_extra
...
Open Hospital's main menu appearance can be customized by adding a logo of choice. To test the feature, rename the image file _logo_hospital_example.png_ found in the _oh/rsc/images_ folder to _logo_hospital.png_:
...
The properties files are related only to the local instance of Open Hospital; different instances may use specific OH configurations with custom properties files.
...
The default configuration is available in the _settings.properties.dist_ file:
...
NOTE: Report translation files are stored in language subfolders under `rpt_base`, `rpt_stat`, and `rpt_extra` (e.g. `rpt_base/es/`).
Installations upgrading from a previous version should migrate their translation files from the legacy naming convention (e.g. `PatientBill_es.properties`) to the new subfolder structure.
A temporary fallback for the legacy structure is available but will be removed in a future version.
...
_AUTOMATIC LOTS MANAGEMENT_ is split into two different settings for charging and discharging the main pharmacy and managing lots also in the wards.
...
By default, PHARMACEUTICALORDER is set to PharmaceuticalOrder, which is the filename of the related report to use for the pharmacy printing functionality.
It is possible to use a different report by installing it in the report folder (see <<reports,Reports>>) and by changing this parameter.
...
By default, PHARMACEUTICALSTOCK is set to PharmaceuticalStock which is the filename of the related report to use for the pharmacy printing functionality.
It is possible to use a different report by installing it in the report folder (see <<reports,Reports>>) and by changing this parameter.
...
Open Hospital allows changing the way the admissions in the Maternity ward are counted within the year; in Open Hospital the first admission of the year for every ward has progressive number 1 (one) and it increments itself automatically up to the end of the year;
...
Open Hospital can merge two patients’ histories into one.
...
NOTE: The DOC_DIR setting can be automatically generated/overwritten at any Open Hospital startup, by setting the WRITE_CONFIG_FILES option to "on" in the **oh.sh / oh.ps1** scripts.
...
see <<log4j2-spring-properties,log4j2-spring.properties>>) and try to use the SetupGSM utility as explained in the next chapter.
...
Open Hospital can drive any webcam with the aim of capturing patient images in the Admission/Patient module
...
Open Hospital can run in debug mode which is useful to understand what is happening behind the user graphical interface.
...
By default, the value of STRONGPASSWORD is set to yes indicating that the
...
|===
|Key |Default Value | Allowed Values
|STRONGLENGTH | 6 | 0 or a positive integer value
|===
...
NOTE: If STRONGPASSWORD is set to *yes*, then STRONGLENGTH should be set to a value greater than or equal to 3 as that is the minimum number of unique characters required.
...
|===
|Key |Default Value | Allowed Values
|PASSWORDTRIES |5 | 0 or a positive integer value greater than or equal to 5
|===

The value of PASSWORDTRIES is the maximum number of consecutive failed passwords when attempting to log in before the account is locked. To disable the checking
of the consecutive failed attempts set PASSWORDTRIES to 0.
...
The value of PASSWORDLOCKTIME is the number of minutes that an account is locked after failing to specify the correct password for the number of times specified by
...
|===
|Key |Default Value | Allowed Values
|PASSWORDIDLE | 365 | a positive integer value greater than or equal to 0; the unit of time is days
|===
...
NOTE: The maximum size for a patient's profile picture is limited to 32,768 bytes. If a bigger image is provided, it will automatically be resized.
...
|===
|Key |Default Value | Allowed Values
|TELEMETRYENABLED |yes | yes, no
|===

By default, the value of TELEMETRYENABLED is the set to `yes` indicating that the Telemetry module is enabled.
...
The Telemetry module can be disabled at any time through the Settings menu or via <<settings-properties,settings.properties>>.

image:image68.png[image,width=831,height=378]

image:image69.png[image,width=847,height=522]
...
<<log4j2-spring-properties,log4j2-spring.properties>> in this manual.
...
The examination module configuration is set in the _examination.properties_ file.
A default configuration file for this module is available in the _examination.properties.dist_ file:
...
The logging configuration is set in the _log4j2-spring.properties_ file.
Default logging configuration is available in the _log4j2-spring.properties.dist_ file:
...
Use the provided OH startup scripts in order to automatically generate the _log4j2-spring.properties_ file from the corresponding _.dist_ file; after generation, optionally adjust the following parameters:
...
NOTE: The _log4j2-spring.properties_ file can be automatically generated/overwritten at any Open Hospital startup, by setting the WRITE_CONFIG_FILES option to "on" in the **oh.sh / oh.ps1** scripts.


NOTE: DEBUG mode can generate large log files and should be avoided in a production environment.
...
The SMS communication module configuration is set in the _sms.properties_ file.
A default configuration file for this module is available in the _sms.properties.dist_ file:
...
The telemetry configuration is set in the _telemetry.properties_ file.
A default configuration file for this module is available in the _telemetry.properties.dist_ file:
...
This file is used only if the TELEMETRYENABLED flag is true (see <<telemetryenabled,TELEMETRYENABLED>> in this document).
...
The text printer configuration is set in the _txtPrinter.properties_ file.
A default configuration file for this module is available in the _txtPrinter.properties.dist_ file:
...
# TXT_CHAR_HEIGHT: height of a text row used by JRTextExporter to convert the
# JRXML layout into a text grid. It should match (or be proportional to)
# the height of the fields in the jrxml report (e.g. 18 in our templates).

# TXT_CHAR_WIDTH: width of a text column used by JRTextExporter. It depends on
# the printable width of the report and determines how many characters fit
# on one line (typically around 42–48 columns for 80mm receipts).

# TXT_FEED_LINES: number of blank lines sent before finishing the print
# TXT_CUTTER: enable/disable the ESC/POS cut command
...
image:image43.png[Accounting_print_as_paid]
...
** *TXT*: the bill is printed as pure text using a monospaced text layout suitable for thermal POS printers
...
* *TXT_CHAR_HEIGHT*: controls the text row height used by the JasperReports TXT exporter. +
This value must be consistent with the JRXML layout to obtain proper line spacing and alignment.
* *TXT_CHAR_WIDTH*: controls the text column width used by the JasperReports TXT exporter. +
This value affects the number of printable characters per line and should be adjusted according to the paper width and font size.
* *TXT_FEED_LINES*: number of empty lines appended at the end of the receipt before closing the print job. +
Useful to improve paper feeding on thermal POS printers.
* *TXT_CUTTER*: if set to _yes_, Open Hospital sends the ESC/POS cutting command after printing. +
If set to _no_, the printer will not attempt automatic cutting. +
Useful for printers without an automatic cutter.
...
NOTE: The best way to configure TXT and ZPL parameters is to adjust them incrementally and print test receipts until the best alignment and readability are achieved for the target printer.
...
The XMPP module configuration is set in the _xmpp.properties_ file.
A default configuration file for this module is available in the _xmpp.properties.dist_ file:
...
Open Hospital is packaged with a login "hint" visible only if the _default_credentials.properties_ file is found.

image:image23.png[image,width=357,height=177]

NOTE: Mouse over the "i" icon to see the hint.

To not show the hint just delete _default_credentials.properties_ file. 

The original values are found in _default_credentials.properties.dist_ and _default_demo_credentials.properties.dist_ files.
...
NOTE: It is always recommended to change the **admin** user password in a production environment.
...
Reports in Open Hospital are produced with JasperReports® technology. JasperReports gives the possibility to design a report in an visual / WYSIWYG editor, connect it to a datasource (DB), test it, modify it, and then compile it to use it directly from the OpenHospital application.

Open Hospital reports are placed into three different folders (see <<folders,Folders>>):

* *rpt_base/* -> contains all the hardcoded reports used by the application;
* *rpt_stat/* -> contains the _OHxxx_ basic reports available in the Statistics menu;
* *rpt_extra/* -> contains additional reports defined by the user.
...
* open the original jrxml file
* modify the report as desired
* save the .jrxml file with a new name
* re-compile the .jrxml file in jasper format (generate the .jasper file)
* place the .jasper file in the report folder used by the Open Hospital application (rpt_extra/)
* Open Hospital will show the custom reports in the Statistics menu.


<<<

== 5 Installing Open Hospital 1.15.0 in Eclipse EE

The OH source code is available on GitHub at the following links:

* https://github.com/informatici/openhospital-core/tree/v1.15.0[openhospital-core]
* https://github.com/informatici/openhospital-gui/tree/v1.15.0[openhospital-gui]
* https://github.com/informatici/openhospital-doc/tree/v1.15.0[openhospital-doc]

_Please use EGit (Eclipse Plugin for Git) to clone the code into the Eclipse instance._

Assuming Eclipse EE, MariaDB/MySQL and the Java Virtual Machine 11 or higher (Java8 or greater)
are already installed on the computer, this chapter will focus on cloning version 1.15.0 into the Eclipse J2EE (or Eclipse EE)
environment.
...
NOTE: Before running the project, the database must be created as explained in the
chapter <<database-server,MySQL Server and Open Hospital Database>>.
...
<OS name> can be “Mac_OS_X”, “Solaris”, “Linux”, “Win64” (for 64bit Windows) or “Windows” (for 32bit Windows)
...
_log4j2-spring.properties_ as explained in the chapter <<log4j2-spring-properties,log4j2-spring.properties>>.
...
(see <<database-properties,database.properties>> and <<log4j2-spring-properties,log4j2-spring.properties>>).
...
. Backup the current database for safety (see <<backup-restore,Backup & Restore>>)
. Open a terminal in the folder sql/update (e.g.):

[source,shell]
----
C:\WINDOWS\system32> cd C:\Users\OH\OpenHospital-v1.15.0\sql\update

C:\Users\OH\OpenHospital-v1.15.0\sql\update>
----
...
[source,shell]
----
C:\Users\OH\OpenHospital-1.15.0\sql\update> mysql –u root -p [-h hostname/ip address]
...
If the database upgrade procedure is successful, it is possible to update the OH software by replacing the OH working copy on each client with the new one.
(see <<database-properties,database.properties>> and <<log4j2-spring-properties,log4j2-spring.properties>>).
...
The default Open Hospital configuration favours ease of setup, not a hardened production deployment. This section collects deployment-level recommendations for installations that are exposed beyond a fully trusted network. They complement the application-level information-disclosure settings in <<settings-properties,settings.properties>> and the API's `application.properties`. These are recommendations, not a turnkey configuration, and should be reviewed against your own environment and threat model.

WARNING: These measures are not a substitute for a full security assessment. The local network should not be treated as a trusted boundary: compromised endpoints, insider access, shared Wi‑Fi or VPN access can all give an attacker internal reachability.

=== 7.1 Web server (reverse proxy)

When the React web UI and the REST API are served through a web server or reverse proxy (for example nginx), harden that layer:

* *Transport security* — serve only over HTTPS with TLS 1.2+ (prefer TLS 1.3), a modern cipher suite and HSTS. The https://ssl-config.mozilla.org/[Mozilla SSL Configuration Generator] produces a suitable configuration for the common web servers.
* *Protocol* — disable obsolete protocols (HTTP/1.0) and, where supported, prefer HTTP/2.
* *Server banner* — suppress the server name and version (`server_tokens off;` in nginx); the API already drops its own version banner (see OP-890).
* *Security response headers* — set at least `Strict-Transport-Security`, `X-Content-Type-Options: nosniff`, `X-Frame-Options` (or a `Content-Security-Policy` `frame-ancestors` directive), `Referrer-Policy` and a restrictive `Permissions-Policy`.
* *File-serving constraints* — deny access to dot-files and sensitive artefacts (`.git`, `.env`, `*.sql`, backups) so they cannot be downloaded.
* *Rate limiting and request limits* — apply request rate limiting and a sensible maximum request/body size to reduce brute-force and abuse.

Illustrative nginx snippet (adapt to your certificates and paths):
...
# do not serve dot-files (except ACME challenges) or database dumps
location ~ /\.(?!well-known) { deny all; }
location ~* \.(sql|env|bak)$ { deny all; }
----

For a complete reference, follow an established guide such as the https://owasp.org/www-project-secure-headers/[OWASP Secure Headers Project] and your web server's own hardening documentation.

=== 7.2 Database

Review the database configuration for a production install:

* *Least privilege* — the application database user should hold only the privileges it needs on the Open Hospital schema; avoid wildcard hosts (`user@'%'`) and administrative grants for the application account.
* *Network exposure* — bind the database to `localhost` (or a private network only) so the port is not reachable from outside the host, and never expose the database port to the internet.
* *Credentials* — change every default password (the database root, the application user and the demo `admin` account) before going to production, and keep the credentials out of shared or version-controlled files.
* *Connection security* — where the database runs on a separate host, use an encrypted connection.

<<<

== 8 Support

For support, or questions about updating, migrating, or maintaining an Open Hospital installation, please check our website at https://www.open-hospital.org.
...
* The application logs (see link:https://github.com/informatici/openhospital-doc/blob/develop/doc_user/UserManual.adoc#help[Help] chapter in the User's Guide)
