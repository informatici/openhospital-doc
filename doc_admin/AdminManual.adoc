...
* Install the Java Runtime Environment (see compatibility matrix for the right version) on every client
...
*on a Linux machine*: during the installation, a “root” passwordi must be defined; it is very important to choose it carefully and to keep it safe. If the password is not set during installation an empty password will be automatically set for the root user.
...
NOTE: **MariaDB installer does not include Bin Directory in Windows PATH; it must be added manually to Windows system variable PATH.**
...
If deployed in a client-server, networked configuration, the database server must be configured to listen on the network interface connected to the local LAN; this can be achieved by editing MariaDB/MySQL's config file, “my.cnf”, and setting the "bind-address" parameter. For example, if you want the database server to listen on all the available network interfaces / IP addresses:
...
[source,shell]
----
C:\Users\OH\OpenHospital-v1.11.0\sql> mysql -u root -p [-h hostname/ip address]
Enter password: *****
----
...
NOTE: Starting with version 1.11, it is also possibile to use the provided scripts (oh.sh/oh.bat) to automatically perform all the database creation tasks.
...
Backup of the client data and configuration files, especially for "PORTABLE" mode, can be performed by saving the "data" folder (including all subfolders) under the main OH installation folder.
...
If PORT is correct but Open Hospital is not able to communicating with the device (try switching to DEBUG log level – or lower - during this setup –
see <<log4j-properties,log4j.properties>>) and try to use the SetupGSM utility (OH startup script with the "G" option: oh.sh -G / oh.ps1 -> select "G").
(see <<folders,Folders>>).
