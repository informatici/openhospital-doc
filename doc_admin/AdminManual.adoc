...
If deployed in a client-server, networked configuration, the database server must be configured to listen on the network interface connected to the local LAN; this can be achieved by editing MariaDB/MySQL's config file, “my.cnf”, and setting the "bind-address" parameter. For example, if you want the database server to listen on all network interfaces / IP addresses:
...
To create the Open Hospital database, a mysql client and set of SQL script files placed under the sql/ folder are needed.
...
now we are ready to create the database user with password, the main OH database and grant the necessary permissions with the following SQL commands:
