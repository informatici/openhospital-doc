...
Most operating systems allow to "open" a downloaded compressed archive by just double-clicking on it. 
In this way, the archive is just opened, but not uncompressed. Let's say it is open for "inspection" only. 
...
TIP: Right-click on the package -> Extract all.../here...

==== 1.6.1 Alternative, uncompress with command line instructions:
...
Open Hospital is a client application that can be used two modalities: 

* <<client,2.1 OH - CLIENT mode>>: multi computers setup, with a central database (requires a little IT knowledge)=
* <<portable,2.2 OH - PORTABLE mode>>: single computer setup, with inbuilt databases (click and run)
...
In this modality, the application will need a Server with a DBMSfootnote:[DataBase Management System] to store the data. 
...
** <<unpack,Unpack>> the downloaded OH package on the server machine by uncompressing the archive in a folder of your choice
...
* Clients
** Unpack the downloaded OH package on a client machine by uncompressing the archive in a folder of your choice
** <<setup,Setup>> and <<run,run>> Open Hospital to point to the OH database server using the hostname or the IP address
** Copy the so configured package on every client
...
More information on these files can be found in the sections under <<configuration,3. Configuration>> section.
