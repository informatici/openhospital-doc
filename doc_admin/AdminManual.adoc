...
Open Hospital is a stand-alone Javafootnote:[Java is a registered trademark of Oracle and/or its affiliates.] software program that does not require installation and is often known as a portable installation. The .zip file can be copied and moved everywhere in your system, whether it is Windows or Linux.
...
WARNING: Beginning with Open Hospital v1.11 Java6 (Java 1.6) is no longer supported. The minimum requirement is Java8 or greater (1.8 or greater).
...
* Copy the Open Hospital application on every client (it may be used on the server too)
* Configure each Open Hospital copy to point the server (using the hostname or the IP address)
...
For the MySQL installation and DB creation refer to the chapter <<mysql-server,MySQL Server and Open Hospital DB>> and to online documentation.

For the Open Hospital installation on every client, it is enough to copy and extract the zip file on the machine.
...
NOTE: Open Hospital v1.10 or earlier versions require a JRE 1.6 to work properly. At the time of the writing of this document, *only JRE 1.6 32bit* has been tested and it is fully compliant with all Open Hospital features, especially the hardware related features (such as Webcam and SMS).
...
In order to create the Open Hospital database everything required is found in a set of SQL script files contained in the Open Hospital ([.underline]#non-portable#) zip file in the folder mysql/.
...
Open Hospital comes with several configuration files called “properties files”. The one we need to modify in order to let Open Hospital find its database is the _database.properties_ and _log4j.properties_ files, located in the “resource” folder rsc/.

We have to specify:

* the *IP address* of the machine where the MySQL Server has been installed and containing the Open Hospital database. Replace “localhost” with the IP address (e.g. 192.168.1.100)
...
Open Hospital, since version 1.2.0, has been distributed in a portable format.
...
WARNING: Beginning with Open Hospital v1.11 Java6 (Java 1.6) is no longer supported. The minimum requirement is Java8 or greater (1.8 or greater).
If you see the application small-scaled, probably you are using an high-dpi display, in such case there are some solutions https://stackoverflow.com/a/39372897/5309541[click here]) to apply only on the machines using this kind of monitors.

* *Portable Open Hospital for Linux* platforms (embedding Java1.8 and MySQL5 for Linux, 32 or 64bit)
* *Portable Open Hospital for Windows* platforms (embedding Java1.8 and MySQL5 for Windows)
...
Backup of the software *code and settings* is needed only if you change it from the “official versions” in order to
...
** *generalData.properties* – for Open Hospital configuration options
...
Open Hospital 1.8 comes with seven translations, identified by the international country code:
...
Open Hospital, since its first version, allows for multi-user usage. It means that is possible to define different users, arrange them by group and set different rights in application usage; for instance, a user “receptionist” may be able to register a new patient but not to prescribe a therapy.
...
Open Hospital, since its first version, allows for  an automatic management of lots in the main pharmacy.
It means that is possible to work in the pharmacy without taking care about lot definitions in loading the store and
...
Open Hospital, since version 1.8, allows managing the cost of medicals in the main pharmacy
(see link:https://github.com/informatici/openhospital-doc/blob/develop/doc_user/UserManual.adoc#6-2-2-2-insert-stock-charging-movement-charge[6.2.2.2 Insert stock charging movement] in the User's Guide).
...
Open Hospital, since its first version, can produce a report about the clinical history of a patient
...
Open Hospital, since its version 1.8, can produce a report about the OPD chart of a patient
(see link:https://github.com/informatici/openhospital-doc/blob/develop/doc_user/UserManual.adoc#10-9-clinical-sheet-clinical-sheet[10.9 Clinical Sheet] in the User’s Guide).
...
Open Hospital, since its version 1.8, can to produce a report about the Discharge of a patient
(see link:https://github.com/informatici/openhospital-doc/blob/develop/doc_user/UserManual.adoc#10-9-clinical-sheet-clinical-sheet[10.9 Clinical Sheet] in the User’s Guide).
...
Open Hospital, since version 1.6.0, can manage patient bills and to produce a report about all bills paid (or not paid) within a span of time (see *8.2 Functions of Accounting* in the _User’s Guide_).
...
Open Hospital, since version 1.6.0, can manage patient bills and to produce a report about all bills paid (or not paid) monthly
(see link:https://github.com/informatici/openhospital-doc/blob/develop/doc_user/UserManual.adoc#8-2-functions-of-accounting[8.2 Functions of Accounting] in the User’s Guide).
...
Open Hospital, since version 1.3.1, can link every OPD visit to a registered patient in order to generate a comprehensive clinical history.
...
Open Hospital, since version 1.4.1, can force every laboratory exam to be linked to a registered patient, in order to have a comprehensive clinical history. For each exam the user will be asked to select a registered patient so from that moment the exam will be attached to the patient's history
...
Open Hospital, since version 1.4.1, can insert multiple laboratory tests per patient at a time in order to avoid the repetitive operation of selecting a patient or writing his/her name; the LABEXTENDED option must be set to yes too
...
Open Hospital, since version 1.4.1, can merge two patient’s history into one.
This is particularly useful in case of double registration where different information was collected over time. Activating this option, the Merge functionality will be available in the Admission/Patient module
(see link:https://github.com/informatici/openhospital-doc/blob/develop/doc_user/UserManual.adoc#10-11-merge-function-merge[Merge function] in the User's Guide).
...
If you are sure the PORT is correct but still Open Hospital is not communicating with the device (we suggest to switch to DEBUG log level – or lower - during this setup –
...
Open Hospital, since version 1.5, can manage text or ZPL printers with aim of printing simple receipts in Accounting module
...
the SINGLEUSER option must be set to no too. If this option is active, Open Hospital will look for an XMPP Server at startup time. The *xmpp.properties* will be used to point the server that must be installed on the computer or on the OH Server by the System Administrator
...
Open Hospital, since version 1.8, embeds a new feature that allows to attach DICOM files to patient’s clinical sheet.
...
Currently this option affects only the video module, that means that in place of the normal *New Patient Extended* window
(see link:https://github.com/informatici/openhospital-doc/blob/develop/doc_user/UserManual.adoc#10-4-3-patient-photo-new-photo[10.4.3 Patient Photo] function in User’s Guide),
...
Open Hospital, since version 1.9, can print the receipt of single open invoice.
...
Open Hospital 1.8 and later versions, comes with the examination.properties file used in the examination module.
...
Starting with Open Hospital version 1.8 a default sms.properties file is included:
...
Starting with Open Hospital version 1.8 a default GSM.properties file is included:
...
Starting with Open Hospital 1.8 a default Skebby.properties file is included:
...
Open Hospital 1.7 and greater versions, come with the xmpp.properties file set as follow by default:
...
In order to use the Communication module
...
<<language,LANGUAGE>> chapter, Open Hospital 1.7 and later versions, come with different available languages in folder *bundles/* within the package file:
...
With iReport® it is possible to edit Open Hospital reports and re-compile them in order to be used in Open Hospital application.
...
WARNING: For previous Open Hospital's versions, in order to have compliant and fully-working reports for Open Hospital you must use iReport® version 3.0.0 (Classic) to edit and compile them.
...
are already installed on the computer, this chapter will focus on cloning the version 1.11 on Eclipse J2EE (or Eclipse EE)
environment.
...
In order to run the application, look for class _openhospital-gui/src/main/java/org/isf/menu/gui/Menu.java_, then right-click on it and choose _Run As -> Java Application_. If everything is OK, the splash screen will appear. If not, look in the Eclipse console for any error or warning messages.

image:image1.jpeg[image,width=390,height=298]

Open Hospitals’s advanced features require “native” libraries generated to match the computer’s operating system. Right-click on project’s name, then select _Run As -> Run Configurations_:
...
according with the folders schema you find in the related folder “native”.
...
Click on the triangle near to the project’s name, do the same on the “rsc” subfolder and then double-click on the <<generaldata-properties,generalData.properties>> file.
...
In order to better understand the processes underneath the software you may also enable the “Standard Output” in
...
. Backup you current DB for safety (see <<backup-restore,Backup & Restore>>)
. Open a terminal in the folder mysql/ (e.g.):
...
----
[start=6]
. We launch all the scripts one by one that differ from previous version; for example:
[source,shell]
----
...
NOTE: Open Hospital portable is not meant to be used in production environment so, if you want to update a portable version in order to keep the previous data, we strongly suggest to switch to a full installation as follows:
...
In order to update the Open Hospital “portable” version without performing a full installation, please contact us at:
info@informaticisenzafrontiere.org and specify:

* Brief history of your installation and use of Open Hospital
* Open Hospital version used and version you need
* Country and language used
* The https://goo.gl/forms/rXgcYTWpAyp9Eyqw1[Open Hospital Assessment online-form] filled.
